...
- `just dev-cp`: runs only `cargo run -p driftbase-controlplane`.
- `just dev-web`: runs only the frontend on `http://127.0.0.1:5173` with `/api` proxied to `:8080`.
- `just dev-agent`: runs the agent; requires `DRIFTBASE_CONTROL_PLANE_URL` plus `DRIFTBASE_BOOTSTRAP_TOKEN` or `DRIFTBASE_NODE_TOKEN`.
...
Required control-plane env vars are `DRIFTBASE_DATABASE_URL` and `DRIFTBASE_MASTER_KEY`; the master key must be base64 for exactly 32 bytes and protects stored credentials and SSH private keys. Treat key rotation as a migration task, not a casual env swap. `DRIFTBASE_PUBLIC_URL`, `DRIFTBASE_BIND_ADDR`, and `DRIFTBASE_COOKIE_SECURE` affect agent registration and cookie behavior. There are two Caddy layers: repo-root Caddy serves the SPA and proxies `/api`, while each managed node runs its own Caddy sidecar for service domains on ports `80/443`. Provisioning is Hetzner-only right now, and services are image-based today; do not document Git-source deploys or true multi-replica reconciliation as if they already exist.

## Commit & Pull Request Guidelines
Follow the existing style: short, imperative, outcome-focused subjects such as `Fix driftbase-caddy restart loop` or `Publish driftbase-web to GHCR`. Keep PRs scoped, explain user-visible or operational impact, call out env or migration changes, and include screenshots for web UI work. If a change touches scheduling, provisioning, domains, or node lifecycle, include rollback or debugging notes.
