...
export const TEMPLATES: Template[] = [
  {
    ...
    name: 'PostgreSQL',
    description:
      'PostgreSQL 16 with persistent storage. Pre-sets PGDATA so a fresh volume initialises cleanly.',
    icon: Database,
    ...
    envVars: [
      {
        ...
        generate: 'password',
        required: true,
        description: 'Superuser password. Generate one — it will only show here once.',
      },
      {
        key: 'POSTGRES_USER',
    ...
    notes:
      'Reachable from other services in this workspace at postgres:5432 on the internal network. The credentials above are what you configured just now.',
  },
  {
    id: 'redis',
    name: 'Redis',
    description: 'Redis 7 with a persistent data volume. No auth — keep it off public ports.',
    icon: ServerCog,
    ...
    volume: { mountPath: '/data', defaultSizeGb: 5 },
    notes:
      'Reachable at redis:6379 inside the workspace network. Authentication is off by default — only expose on domains if you want public access (and enable auth if you do).',
  },
];
